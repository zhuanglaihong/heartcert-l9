...
_MOCK_CANDIDATES = [str(uuid.uuid4()) for _ in range(500)]
...
def _mock_recall_l1(target_vec_32: list[float], limit: int = 200) -> list[tuple[str, int]]:
    """
    L1 粗召回：32D HNSW cosine 相似度。
...
def _mock_recall_l2(
    ...
    subset = [c for c in candidate_ids if _RNG.random() < 0.85][:limit]
    _RNG.shuffle(subset)
...
def _mock_recall_l3(
    ...
    subset = [c for c in candidate_ids if _RNG.random() < 0.80][:limit]
    _RNG.shuffle(subset)
    return [(c, rank + 1) for rank, c in enumerate(subset)]


def _mock_sparse_recall(tags: list[str], limit: int = 40) -> list[tuple[str, int]]:
    """
    稀疏补漏：GIN 索引 BM25 匹配 profile_data.verified_skills。
    仅作为 L1/L2/L3 的补充路径，防止文本噪声主导排名。
    """
    candidates = _MOCK_CANDIDATES[:]
    _RNG.shuffle(candidates)
    return [(c, rank + 1) for rank, c in enumerate(candidates[:limit])]
...
def _mock_fetch_reranker_payloads(candidate_ids: list[str]) -> dict[str, str]:
    """
    ...
    return {
        cid: (
            ...
            f"处理了 Redis 宕机和 goroutine 泄漏，耗时约 {_RNG.randint(8, 25)} 分钟，"
...
def _mock_cross_encoder_rerank(
    ...
    for cid, payload in payloads.items():
        ...
        # 模拟 logit → sigmoid 归一化
        raw_logit = overlap / max(len(query_tokens), 1) + _RNG.uniform(0.0, 0.2)
        score = 1.0 / (1.0 + 2.718 ** (-raw_logit * 3))
...
async def search(req: SearchRequest) -> SearchResponse:
    """
    B 端混合检索主入口。
    完整链路：Filter → L1(32D) → L2(128D) → L3(1024D) → Sparse → RRF → Top30 → Rerank → TopK
    """
    ...
    # ── Step 2: 三层并发召回（L1 / L2 / L3 / Sparse 四路并发）─────────────
    t0 = time.perf_counter()
    (l1_results, sparse_results) = await asyncio.gather(
        asyncio.to_thread(_mock_recall_l1, parsed["target_vec_32"], limit=200),
        asyncio.to_thread(_mock_sparse_recall, parsed["extracted_tags"], limit=40),
    )
    l1_ids = [c for c, _ in l1_results]

    # L2 仅在 L1 结果集上运行（减少范围）
    l2_results = await asyncio.to_thread(
        _mock_recall_l2, parsed["target_vec_128"], l1_ids, limit=80
    )
    l2_ids = [c for c, _ in l2_results]

    # L3 仅在 L2 结果集上运行
    l3_results = await asyncio.to_thread(
        _mock_recall_l3, parsed["target_vec_1024"], l2_ids, limit=40
    )
    ...
    t0 = time.perf_counter()
    payloads = await asyncio.to_thread(_mock_fetch_reranker_payloads, top30_ids)
    latencies["payload_fetch_ms"] = round((time.perf_counter() - t0) * 1000, 2)

    # ── Step 5: Cross-Encoder 重排（BGE-Reranker 深度注意力计算）───────────
    t0 = time.perf_counter()
    reranked = await asyncio.to_thread(
        _mock_cross_encoder_rerank, req.query_text, payloads, top_k=req.top_k
    )
    ...
    for cid, rerank_score in reranked:
        results.append(CandidateScore(
            ...
            rrf_score=round(fused.get(cid, 0.0), 5),
            rerank_score=rerank_score,
            final_score=round(0.4 * fused.get(cid, 0.0) + 0.6 * rerank_score, 5),
    ...
    if total_ms > 1500:
        # 超 1.5s 告警（生产环境应接入监控）
        import logging
        logging.warning(f"[search] session={session_id} latency={total_ms}ms exceeds 1500ms SLA")
